      ...
      m_comp_R(solution.data() + prob.nz + prob.n_ineq + prob.n_comp + prob.n_eq + prob.n_ineq + prob.n_comp, prob.n_comp)
{
    ...
    m_comp_L_est.resize(prob.n_comp);
    m_comp_R_est.resize(prob.n_comp);
    ...
    backsolve_scratch.resize(n_vars);

    residual_eq.resize(prob.n_eq);
    residual_ineq.resize(prob.n_ineq);
    residual_comp_L.resize(prob.n_comp);
    residual_comp_R.resize(prob.n_comp);
    ...
    residual_comp_L = prob.L_comp * z + prob.l_comp;
    residual_comp_R = prob.R_comp * z + prob.r_comp;
